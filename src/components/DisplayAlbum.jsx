import React from 'react'
import Navbar from './Navbar'
import {useParams} from 'react-router-dom'
import { albumsData , assets, songsData} from '../assets/assets'
import { PlayerContext } from '../context/Playercontext'
import { useContext } from 'react'
function DisplayAlbum() {
    const {id} =useParams()
    const albumData = albumsData[id] 
    const {playwithid} = useContext(PlayerContext)
  //  console.log(albumData)
   return (
    <>
    <div>
        <Navbar/>
        <div className='mt-10 flex gap-8 flex-col md:flex-row md:items-end'>
          <img  className='w-48 rounded'src={albumData.image} alt="" />
          <div className='flex flex-col'>
           <p className='font-semibold'>Playlist</p>
           <h2 className='text-5xl font-bold mb-4 md:text-7xl'>{albumData.name}</h2>
           <h4>{albumData.desc}</h4>
           <p className='mt-1 items-center flex gap-1'>
            <img src={assets.spotify_logo} alt=""  className='inline-block w-5 '/>
            <b> Spotify</b>
            <b> 50 songs, </b>
            about 2hr 30 min
           </p>
          </div>
        </div>
        <div className='grid grid-cols-3 sm:grid-cols-4 mt-10 mb-4 pl-2 text-white'>
          <p><b className='mr-4 '>#</b>Title</p>
          <p>album</p>
          <p className='hidden sm:block'>Date-added</p>
          <img src={assets.clock_icon} alt="" className='w-4 m-auto' />

        </div>
        <hr />
        {
          songsData.map((item,index)=>(
            <div key={index} onClick={()=>playwithid(item.id)} className=' grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-white hover:bg-[#ffffff2b] cursor-pointer'>
               <p className='text-white'>
                    <b className='mr-4 text-white'>{index+1}</b>
                    <img src={item.image} alt="" className='inline w-10 mr-5' />
                    {item.name}
               </p>
               <p className='text-[15px]'>{albumData.name}</p>
               <p className='text-[15px] hidden sm:block'> 5days ago</p>
               <p className='text-[15px] text-center'>{item.duration}</p>
            </div>
          ))
        }
    </div>
    </>
  )
}

export default DisplayAlbum